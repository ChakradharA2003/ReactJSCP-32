...
class Events extends Component {
  ...
  onClickImageEvent = id => {
    console.log(id)
    const filterEvents = eventsList.filter(event => event.id === id)
    console.log(filterEvents[0].registrationStatus)
    this.setState({registrationStatus: filterEvents[0].registrationStatus})
  }
          ...
          <ul className="events-list">
            {eventsList.map(event => (
              <EventItem
                key={event.id}
                details={event}
                onClickImageEvent={this.onClickImageEvent}
              />
            ))}
          </ul>
        </div>
